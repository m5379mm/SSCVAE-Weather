...
class AttentiveLISTA_Time(nn.Module):
    def __init__(self,
        ...
        self._time_attention = TemporalAttention(num_steps=1)
    ...
    def forward(self, x):
        ...
        """
        B, T, C, H, W = x.size()
        # print(x.size(), "Input Shape")  # 查看输入形状
        ...
        # print(l.size(), "After Reshaping to [B, T, K, H, W]")  # 查看重塑后的形状

        # 4. 时间注意力加权
        self._time_attention.num_steps = T  # 动态设置 num_steps
        l = self._time_attention(l)  # [B, T, K, H, W]
        # print(l.size(), "After Time Attention")  # 查看时间注意力后的形状
...
class SSCVAEDouble(nn.Module):
    ...
    #     # 1) 编码到中间表征
    #     ex_sate_069  = self._encoder_sate_r(sat_btchw[:,:,0,:,:])   # [B,T,D,h,w]
    #     ex_sate_107  = self._encoder_sate_g(sat_btchw[:,:,1,:,:])   # [B,T,D,h,w]
    #     ex_sate_lght  = self._encoder_sate_b(sat_btchw[:,:,2,:,:])   # [B,T,D,h,w]
    #     ex_radar_seq = self._encoder_radar(radar_btchw)  # [B,T,D,h,w]

    #     # 2) LISTA 稀疏编码
    #     # print(ex_sate_seq.size())
    #     z_sate_seq,  exrec_sate_seq,  dictionary = self._LISTA(ex_sate_069)   # [B,T,K,h,w], [B,T,D,h,w]
    #     z_radar_seq, exrec_radar_seq, _         = self._LISTA(ex_radar_seq)   # [B,T,K,h,w], [B,T,D,h,w]
...
def segmented_weighted_loss(x_recon, x_target):
    ...
    weights[(x_target_01 >= 16/255) & (x_target_01 < 181/255)] = 1.0
    weights[(x_target_01 >= 181/255) & (x_target_01 < 219/255)] = 4.0
    weights[(x_target_01 >= 219/255)] = 8.0

    # ---- 加权 MSE ----
    diff = (x_recon - x_target) ** 2
...
# 时序膨胀压缩
class SparseU_Ne7t(nn.Module):
    def __init__(self, in_channels, out_channels, num_res_blocks=3):
        super(SparseU_N7et, self).__init__()
...
class SparseU_Net8(nn.Module):
    def __init__(self, in_channels, out_channels, num_res_blocks=3):
        super(SparseU_Net8, self).__init__()
...
def ssim(img1, img2, window_size=11, window=None, size_average=True, val_range=255):
    # img1, img2: [B, 1, H, W]
    if img1.is_cuda:
...
def focal_loss(x_recon, x_target, alpha=1.0, gamma=2.0, threshold=0.5):
    """
    x_recon, x_target: shape [B, C, T, H, W]
    alpha: 平衡因子，用于调整不同区域的权重
    gamma: 焦点因子，用于增强难预测区域的损失
    threshold: 强信号的阈值 (例如 0.71 对应 dBZ 181)
    """
    # 标准化到 [0, 1]
    x_target_01 = x_target
    if x_target.max() > 1:
        x_target_01 = x_target / 255.0
    if x_recon.max() > 1:
        x_recon = x_recon / 255.0
    ...
    # 计算焦点因子 (对强信号区域放大损失)
    p_t = torch.abs(x_recon - x_target_01)  # 可以是残差，也可以是预测值与目标的绝对差
    focal_factor = (1 - p_t) ** gamma

    # 只对强信号区域应用加权
    weight_mask = (x_target_01 >= threshold).float() * alpha + (x_target_01 < threshold).float()
    weighted_loss = weight_mask * focal_factor * diff
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=1.0, beta=0.4, gamma=0.2, focal_alpha=1.0, focal_gamma=2.5, threshold=0.71):
        """
        alpha: segmented_weighted_loss 权重
        beta : SSIM loss 权重
        gamma: Edge loss 权重
        focal_alpha: Focal loss 的平衡因子
        focal_gamma: Focal loss 的焦点因子
        threshold: 强信号的阈值 (例如 0.71 对应 dBZ 181)
        """
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        # 2) segmented_weighted_loss
        mse_loss_val = segmented_weighted_loss(pred, target)

        # 3) SSIM
        pred_2d = pred.permute(0, 2, 1, 3, 4).reshape(pred.shape[0]*pred.shape[2], pred.shape[1], pred.shape[3], pred.shape[4])
        target_2d = target.permute(0, 2, 1, 3, 4).reshape(target.shape[0]*target.shape[2], target.shape[1], target.shape[3], target.shape[4])

        val_range = 1 if target_2d.max() <= 1 else 255
        window = create_window(11, pred.shape[1]).to(pred.device)
        ssim_val = ssim(pred_2d, target_2d, window_size=11, window=window, val_range=val_range)
        ssim_loss = 1 - ssim_val

        # 4) Edge loss
        pred_edge = sobel_edges(pred_2d.mean(1, keepdim=True))
        target_edge = sobel_edges(target_2d.mean(1, keepdim=True))
