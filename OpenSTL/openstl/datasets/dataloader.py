...
def load_data(dataname, batch_size, val_batch_size, num_workers, data_root, dist=False, **kwargs):
    ...
    elif 'sevir_raw' in dataname:  # 'sevir_raw' - 原始 H5 文件，支持7帧→6帧
        from .dataloader_sevir_raw import load_data
        cfg_dataloader['input_frames'] = kwargs.get('input_frames', 7)
        cfg_dataloader['output_frames'] = kwargs.get('output_frames', 6)
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
