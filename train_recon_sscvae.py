...
from data import SevirTrans0Dataset
...
if data_args.dataset == 'gray':
    ...
    train_dataset = MNISTDataset(root_dir=data_args.root_dir,
                                 mode="train",
                                 transform=data_transform["train"])
    val_dataset = MNISTDataset(root_dir=data_args.root_dir,
                               mode="val",
                               transform=data_transform["val"])
elif data_args.dataset == 'sevir':
    train_dataset = SevirTrans0Dataset(root_dir=data_args.root_dir,
                                 mode="train",
                                 transform=data_transform["train"])
    val_dataset = SevirTrans0Dataset(root_dir=data_args.root_dir,
                               mode="val",
                               transform=data_transform["val"])
else:
...
with open(csv_filename, 'w', newline='') as csvfile:
    fieldnames = ['Epoch', 'Train Recon Loss', 'Train Latent Loss',  'Train Latent Dist Loss','Train Total Loss', 'Train Sparsity',
                           'Val Recon Loss', 'Val Latent Loss', 'Val Latent Dist Loss', 'Val Total Loss', 'Val Sparsity']
...
for epoch in range(train_args.epochs):
    ...
    for batch_idx, (satellite, vil) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}", ncols=100)):
        ...
        #print(f"Batch {batch_idx + 1}, Batch size: {satellite.size(0)}")
        # get( the inputs
        #print(index)
        #index+=1
        ...
        # forward
        # image_recon, z, latent_loss, _ = model(satellite,vil)
    ...
    with torch.no_grad():
        for batch_idx, (satellite, vil) in enumerate(tqdm(val_loader, desc=f"Validation Epoch {epoch+1}", ncols=100)):
            ...
            # forward
            x_recon_trans, z, latent_loss,latent_dist_loss,reconstruction_loss, _ = model(satellite,vil)
            sparsity = hoyer_metric(z)
            #recon_loss = get_recon_loss(image_origin, image_recon)
            #sparsity = hoyer_metric(z)
